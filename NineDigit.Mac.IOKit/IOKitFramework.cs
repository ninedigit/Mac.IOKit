...
namespace NineDigit.Mac.IOKit
{
	...
	public static class IOKitFramework
	{
		...
		public static IOIterator? FindModems()
		{
			...
			{
				if (SerialPort.Verbosity > TraceVerbosity.Silent)
				{
		...
		public static int GetCFPropertyInt(IOObject dictionary, string key, out int value)
		{
			...
			if (bsdPathAsCFValue != IntPtr.Zero)
			{
				...
				try
				{
					...
					var result = NativeMethods.CFNumberGetValue(bsdPathAsCFValue, CFNumberType.kCFNumberIntType,
						out bsdValue);
		...
		private unsafe static class NativeMethods
		{
			...
			[DllImport(IOKitFrameworkPath, CharSet = CharSet.Ansi)]
			public static extern int IOServiceGetMatchingServices(IntPtr masterPort, IntPtr matching,
				out IntPtr iterator);
			...
			[DllImport(IOKitFrameworkPath, CharSet = CharSet.Ansi)]
			public static extern int IORegistryEntryCreateIterator(IntPtr entry, string plane, uint options,
				out IntPtr iterator);
			...
			public static int FindModems(ref IntPtr matchingServices)
			{
				...
				CFDictionarySetValue(classesToMatch, __CFStringMakeConstantString(kIOSerialBSDTypeKey),
					__CFStringMakeConstantString(kIOSerialBSDAllTypes));

				// Get an iterator across all matching devices.
				var kernResult =
					IOServiceGetMatchingServices(kIOMasterPortDefault, classesToMatch, out matchingServices);
				if (KERN_SUCCESS != kernResult)
				{
					if (SerialPort.Verbosity > TraceVerbosity.Silent)
					{
			...
			public static int GetModemPath(IntPtr modemService, out string path)
			{
				...
				var bsdPathAsCFString = IORegistryEntryCreateCFProperty(modemService,
					__CFStringMakeConstantString(kIOCalloutDeviceKey), kCFAllocatorDefault, 0);
				...
				if (bsdPathAsCFString != IntPtr.Zero)
				{
					...
					try
					{
						unsafe
						{
							...
							{
								var result = CFStringGetCString(bsdPathAsCFString, bsdPath, maxPathSize,
									CFStringEncoding.kCFStringEncodingUTF8);
								...
								if (result)
								{
									...
									if (SerialPort.Verbosity > TraceVerbosity.ErrorAndWarning)
									{
			...
			public static IntPtr GetUsbDevice(IntPtr modemService)
			{
				...
				var status = IORegistryEntryCreateIterator(modemService, kIOServicePlane,
					kIORegistryIterateParents | kIORegistryIterateRecursively, out iterator);
